...
describe 'building a site with jekyll-md' do
  ...
  context 'with md.title_heading enabled' do
    before(:all) do # rubocop:disable RSpec/BeforeAfterAll
      FileUtils.rm_rf(File.join(fixture_site_path, '_site'))
      @site = build_fixture_site('md' => { 'selector' => '#markdown-content', 'title_heading' => true })
    end
    ...
    it 'prepends a title heading to the markdown output' do
      markdown = site_file('index.md')
      expect(markdown).to start_with("# Home\n\n")
    end
  end
end
