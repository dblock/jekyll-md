...
describe 'building a site with jekyll-md' do
  ...
  context 'with md.frontmatter enabled' do
    before(:all) do # rubocop:disable RSpec/BeforeAfterAll
      FileUtils.rm_rf(File.join(fixture_site_path, '_site'))
      @site = build_fixture_site('md' => { 'selector' => '#markdown-content', 'frontmatter' => %w[title url] })
    end
    ...
    it 'prepends a YAML front matter block to the markdown output' do
      markdown = site_file('index.md')
